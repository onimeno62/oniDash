...
public static class MusicEndpoints
{
    ...
    public static IEndpointRouteBuilder MapMusicEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        music.MapGet("/artists", async (
            MusicDbContext db, Guid? libraryId, string? q, string? sort, bool? desc,
            int? limit, int? offset, CancellationToken ct) =>
        {
            ...
            {
                "tracks" => query.OrderByDescending(a => a.Id),
                _ => query.OrderBy(a => a.Name),
            };
            if (desc == true) ordered = query.OrderByDescending(a => a.Name);

            var items = await ordered.ThenBy(a => a.Id).Skip(skip).Take(take)
                .Select(a => new MusicArtistDto(a.Id, a.LibraryId, a.Name,
                    db.Tracks.Count(t => t.ArtistId == a.Id || t.AlbumArtistId == a.Id),
        ...
        music.MapGet("/albums", async (
            MusicDbContext db, Guid? libraryId, Guid? artistId, string? q, int? year,
            bool? missing, string? sort, bool? desc, int? limit, int? offset, CancellationToken ct) =>
        {
            var (take, skip) = Page(limit, offset);
            ...
            var ordered = sort?.ToLowerInvariant() switch
            {
                "year" => query.OrderBy(a => a.Year).ThenBy(a => a.Title),
                "artist" => query.OrderBy(a => a.ArtistName).ThenBy(a => a.Title),
                _ => query.OrderBy(a => a.Title),
            };
            if (desc == true) ordered = sort?.ToLowerInvariant() == "year"
                ? query.OrderByDescending(a => a.Year).ThenByDescending(a => a.Title)
                : query.OrderByDescending(a => a.Title);

            var items = await ordered.ThenBy(a => a.Id).Skip(skip).Take(take)
                .Select(a => new MusicAlbumDto(a.Id, a.LibraryId, a.Title, a.ArtistName, a.Year,
                    a.CoverBlob != null, db.Tracks.Count(t => t.AlbumId == a.Id && !t.IsMissing)))
                .ToListAsync(ct);
            return Results.Ok(new MusicPage<MusicAlbumDto>(items, skip, take, total));
        });

        music.MapGet("/tracks", async (
            MusicDbContext db, Guid? libraryId, Guid? albumId, Guid? artistId, string? q,
            string? genre, bool? missing, bool? favorite, int? ratingMin, string? sort, bool? desc,
            int? limit, int? offset, CancellationToken ct) =>
        {
            var (take, skip) = Page(limit, offset);
            ...
            {
                "added" => query.OrderBy(t => t.AddedAtUtc),
                "updated" => query.OrderBy(t => t.UpdatedAtUtc),
                "rating" => query.OrderBy(t => t.Rating),
                "year" => query.OrderBy(t => t.Year),
                "duration" => query.OrderBy(t => t.DurationSeconds),
                "artist" => query.OrderBy(t => t.ArtistName),
                "album" => query.OrderBy(t => t.AlbumArtistName).ThenBy(t => t.AlbumId),
            ...
            if (desc == true)
            {
                ordered = key switch
                {
                    "added" => query.OrderByDescending(t => t.AddedAtUtc),
                    "updated" => query.OrderByDescending(t => t.UpdatedAtUtc),
                    "rating" => query.OrderByDescending(t => t.Rating),
                    "year" => query.OrderByDescending(t => t.Year),
                    "duration" => query.OrderByDescending(t => t.DurationSeconds),
                    "artist" => query.OrderByDescending(t => t.ArtistName),
                    "album" => query.OrderByDescending(t => t.AlbumArtistName).ThenByDescending(t => t.AlbumId),
            ...
            var items = await ordered.ThenBy(t => t.Id).Skip(skip).Take(take)
                .Select(t => new MusicTrackDto(
                    t.Id, t.LibraryId, t.MediaItemId, t.FileId, t.Title, t.ArtistName, t.AlbumArtistName,
                    t.Album != null ? t.Album.Title : null, t.AlbumId, t.TrackNumber, t.DiscNumber, t.Year,
                    t.DurationSeconds, t.Genre, t.Rating,
                    db.Favorites.Any(f => f.EntityType == "track" && f.EntityId == t.Id),
                    t.IsMissing, t.AddedAtUtc, t.UpdatedAtUtc))
                .ToListAsync(ct);
        ...
        music.MapGet("/genres", async (MusicDbContext db, Guid? libraryId, int? limit, int? offset, CancellationToken ct) =>
        {
            ...
            var query = db.Tracks.AsNoTracking()
                ...
                .SelectMany(t => t.Genre!.Split(';', StringSplitOptions.RemoveEmptyEntries).AsQueryable());
            var items = await query.GroupBy(g => g).Select(g => new { Genre = g.Key, TrackCount = g.Count() })
                .OrderBy(g => g.Genre).Skip(skip).Take(take).ToListAsync(ct);
            return Results.Ok(items);
        });

        music.MapGet("/tracks/{trackId:guid}", async (MusicDbContext db, Guid trackId, CancellationToken ct) =>
        {
            var item = await db.Tracks.AsNoTracking()
                .Where(t => t.Id == trackId)
                .Select(t => new MusicTrackDto(t.Id, t.LibraryId, t.MediaItemId, t.FileId, t.Title,
                    t.ArtistName, t.AlbumArtistName, t.Album != null ? t.Album.Title : null, t.AlbumId,
                    t.TrackNumber, t.DiscNumber, t.Year, t.DurationSeconds, t.Genre, t.Rating,
                    db.Favorites.Any(f => f.EntityType == "track" && f.EntityId == t.Id),
                    t.IsMissing, t.AddedAtUtc, t.UpdatedAtUtc))
        ...
        music.MapPost("/reindex", async (MusicReindexService reindex, Guid? libraryId, CancellationToken ct) =>
            Results.Ok(new { indexedTracks = await reindex.ReindexAsync(libraryId, ct) }));

        return app;
    }

    private static (int Take, int Skip) Page(int? limit, int? offset) =>
        (Math.Clamp(limit ?? DefaultLimit, 1, MaxLimit), Math.Max(0, offset ?? 0));
}
