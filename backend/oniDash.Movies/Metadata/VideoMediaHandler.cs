...
/// <summary>Adapts local video probe and embedded artwork results to the platform contract.</summary>
public sealed class VideoMediaHandler(IVideoProbeReader reader, IVideoArtworkReader? artworkReader = null) : IMediaHandler
{
    private static readonly IReadOnlySet<MediaType> Types = new HashSet<MediaType> { MediaType.Video };
    private static readonly IReadOnlySet<string> Extensions = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { ".mp4", ".mkv", ".avi", ".mov", ".webm" };
    public string Id => "movies.video";
    public IReadOnlySet<MediaType> SupportedTypes => Types;
    public bool CanHandle(MediaDescriptor descriptor) => descriptor.MediaType == MediaType.Video && Extensions.Contains(NormalizeExtension(descriptor.Extension));
    ...
    public async Task<MediaInspectionResult> InspectAsync(MediaDescriptor descriptor, CancellationToken cancellationToken = default)
    {
        ...
        var artwork = Array.Empty<ArtworkCandidate>();
        if (artworkReader is not null)
        {
            var embedded = await artworkReader.ReadAsync(descriptor.AbsolutePath, cancellationToken).ConfigureAwait(false);
            if (embedded is { Bytes.Length: > 0 } && !string.IsNullOrWhiteSpace(embedded.ContentType))
