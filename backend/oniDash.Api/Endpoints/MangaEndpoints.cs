...
public static class MangaEndpoints
{
    ...
    private static MangaDto ToDto(MediaItem item, MangaState? state)
    {
        ...
        return new MangaDto(item.Id.ToString(), item.DisplayName, null, null, null, null, $"/api/manga/{item.Id}/cover", null, item.Tags.Select(x => x.Tag.Name).ToArray(), state?.Status ?? "plan_to_read", chapters, Math.Max(0, chapters - read), read, state?.LastReadAtUtc, null, null, state?.Favorite ?? false, true, state?.DownloadCount ?? 0, progress, null, DateTimeOffset.UtcNow);
    }
