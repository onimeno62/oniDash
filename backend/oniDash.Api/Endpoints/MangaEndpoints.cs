...
/// <summary>Local Manga catalogue API backed by indexed media files and persistent reading state.</summary>
public static class MangaEndpoints
{
    ...
    public static IEndpointRouteBuilder MapMangaEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        manga.MapGet("/library", async (OniDashDbContext db, int? limit, CancellationToken ct) => Results.Ok(await Load(db, null, limit ?? 500, ct)));
        manga.MapGet("/continue-reading", async (OniDashDbContext db, int? limit, CancellationToken ct) => { var items = await Load(db, null, 500, ct); return Results.Ok(items.Where(x => x.ProgressPercent > 0 && x.ProgressPercent < 100).OrderByDescending(x => x.LastReadAtUtc).Take(Math.Clamp(limit ?? 12, 1, 100))); });
        manga.MapGet("/updates", async (OniDashDbContext db, int? limit, CancellationToken ct) => { var items = await Load(db, null, 500, ct); return Results.Ok(items.OrderByDescending(x => x.UpdatedAtUtc).Take(Math.Clamp(limit ?? 50, 1, 200)).Select(x => new MangaUpdateDto(x.Id, x.Id, x.Title, x.CoverUrl, x.LatestChapter ?? "New", x.UpdatedAtUtc, x.SourceName, false, x.ProgressPercent >= 100))); });
        manga.MapGet("/categories", async (OniDashDbContext db, CancellationToken ct) =>
        {
            var tags = await db.Tags.AsNoTracking().Where(t => t.Items.Any(i => i.MediaItem.Files.Any(f => MangaExtensions.Contains(f.Extension)))).Select(t => new { t.Id, t.Name, Count = t.Items.Count(i => i.MediaItem.Files.Any(f => MangaExtensions.Contains(f.Extension))) }).OrderByDescending(x => x.Count).ToListAsync(ct);
            return Results.Ok(tags.Select(x => new MangaCategoryDto(x.Id.ToString(), x.Name, x.Count, null)));
        });
        ...
        manga.MapGet("/browse/popular", async (OniDashDbContext db, int? limit, CancellationToken ct) => Results.Ok((await Load(db, null, 500, ct)).OrderByDescending(x => x.ChapterCount).Take(Math.Clamp(limit ?? 24, 1, 100))));
        manga.MapGet("/search", async (OniDashDbContext db, string? q, int? limit, CancellationToken ct) => Results.Ok((await Load(db, null, 2000, ct)).Where(x => string.IsNullOrWhiteSpace(q) || x.Title.Contains(q, StringComparison.OrdinalIgnoreCase) || (x.Author?.Contains(q, StringComparison.OrdinalIgnoreCase) ?? false)).Take(Math.Clamp(limit ?? 50, 1, 500))));
    ...
    private static async Task<List<MangaDto>> Load(OniDashDbContext db, Guid? libraryId, int limit, CancellationToken ct)
    {
        await EnsureStateTable(db, ct);
        var query = db.MediaItems.AsNoTracking().Include(x => x.Files).Include(x => x.Artwork).Include(x => x.Tags).ThenInclude(x => x.Tag).Where(x => x.Files.Any(f => MangaExtensions.Contains(f.Extension)));
        if (libraryId is { } id) query = query.Where(x => x.LibraryId == id);
        var items = await query.OrderBy(x => x.DisplayName).Take(Math.Clamp(limit, 1, 2000)).ToListAsync(ct);
        var states = await ReadStates(db, items.Select(x => x.Id).ToArray(), ct);
        return items.Select(x => ToDto(x, states.GetValueOrDefault(x.Id))).ToList();
    }

    private static MangaDto ToDto(MediaItem item, MangaState? state)
    {
        var file = item.Files.OrderBy(x => x.RelativePath.Length).FirstOrDefault(); var chapters = item.Files.Count; var progress = state?.ProgressPercent ?? 0;
        var cover = item.Artwork.FirstOrDefault()?.SourcePath;
        return new MangaDto(item.Id.ToString(), item.DisplayName, null, null, cover, null, item.Tags.Select(x => x.Tag.Name).ToArray(), state?.Status ?? "plan_to_read", chapters, Math.Max(0, chapters - (int)Math.Round(chapters * progress / 100)), chapters == 0 ? 0 : (int)Math.Round(chapters * progress / 100), null, state?.Favorite ?? false, true, chapters, progress, state?.LastReadAtUtc, file?.LibrarySourceId.ToString(), null, DateTimeOffset.UtcNow);
    }
    ...
    {
        await EnsureStateTable(db, ct); var current = (await ReadStates(db, new[] { id }, ct)).GetValueOrDefault(id) ?? new MangaState(false, "plan_to_read", 0, null, 0);
        var nextProgress = progress is null ? current.ProgressPercent : Math.Min(100, current.ProgressPercent + 10);
    ...
    private static async Task<Dictionary<Guid, MangaState>> ReadStates(OniDashDbContext db, Guid[] ids, CancellationToken ct)
    {
        ...
        command.CommandText = "SELECT id, favorite, status, progress_percent, last_read_at_utc, download_count FROM manga_catalog_state WHERE id IN (" + string.Join(',', ids.Select((_, i) => "@p" + i)) + ")";
        for (var i = 0; i < ids.Length; i++) command.Parameters.Add(new SqliteParameter("@p" + i, ids[i].ToString())); await using var reader = await command.ExecuteReaderAsync(ct); var result = new Dictionary<Guid, MangaState>();
        while (await reader.ReadAsync(ct)) result[Guid.Parse(reader.GetString(0))] = new MangaState(reader.GetInt64(1) != 0, reader.GetString(2), reader.GetDouble(3), reader.IsDBNull(4) ? null : DateTimeOffset.Parse(reader.GetString(4)), reader.GetInt32(5));
        return result;
    }
    ...
    private static async Task UpsertState(OniDashDbContext db, Guid id, bool favorite, string status, double progress, DateTimeOffset? lastReadAtUtc, int downloadCount, CancellationToken ct)
    {
        ...
        command.Parameters.Add(new SqliteParameter("@id", id.ToString())); command.Parameters.Add(new SqliteParameter("@favorite", favorite ? 1 : 0)); command.Parameters.Add(new SqliteParameter("@status", status)); command.Parameters.Add(new SqliteParameter("@progress", progress)); command.Parameters.Add(new SqliteParameter("@last", (object?)lastReadAtUtc?.ToString("O") ?? DBNull.Value)); command.Parameters.Add(new SqliteParameter("@downloads", downloadCount)); await command.ExecuteNonQueryAsync(ct);
    }
    ...
    public sealed record MangaDto(string Id, string Title, string? AltTitle, string? Author, string? CoverUrl, string? SourceId, string[] Genres, string Status, int ChapterCount, int UnreadCount, int ReadCount, string? LastReadAtUtc, bool Favorite, bool InLibrary, int DownloadCount, double ProgressPercent, DateTimeOffset? LastReadAtUtcValue, string? SourceName, string? LatestChapter, DateTimeOffset UpdatedAtUtc)
    {
        public DateTimeOffset? LastReadAtUtc => LastReadAtUtcValue;
    }
