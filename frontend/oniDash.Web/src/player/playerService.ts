import { recordMusicPlay, saveMusicPlaybackState, type TrackSummary } from '../api/music';
...
class PlayerService {
  ...
  private persistPlayback(force: boolean) {
    ...
    this.persist();
    void saveMusicPlaybackState(this.state.current.id, this.state.position, false).catch(() => undefined);
  }
  ...
  playQueue(tracks: TrackSummary[], index = 0) {
    if (!tracks.length) return;
    const safe = Math.max(0, Math.min(index, tracks.length - 1));
    this.state = { ...this.state, queue: tracks, queueIndex: safe, current: tracks[safe], position: 0, error: null };
    this.loadCurrent(true);
  }
  toggle(track: TrackSummary) {
    if (this.state.current?.id === track.id) { if (this.audio.paused) void this.audio.play().catch(() => this.patch({ error: 'Playback was blocked by the browser.' })); else this.audio.pause(); return; }
    const index = this.state.queue.findIndex(item => item.id === track.id);
    this.playQueue(index >= 0 ? this.state.queue : [track], index >= 0 ? index : 0);
  }
  private loadCurrent(autoplay: boolean) {
    ...
    this.patch({ duration: track.durationSeconds ?? 0, error: null });
  ...
  async advance(fromEnded = false) {
    ...
    if (fromEnded && this.state.current) {
      const played = Math.max(0, this.state.position);
      void recordMusicPlay(this.state.current.id, played, this.state.duration > 0 ? Math.min(1, played / this.state.duration) : 1).catch(() => undefined);
      void saveMusicPlaybackState(this.state.current.id, this.state.duration || played, true).catch(() => undefined);
    }
