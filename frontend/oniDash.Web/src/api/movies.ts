...
/** A movie (mirrors oniDash.Movies.Endpoints.MovieEndpoints.MovieSummary). */
...
export function fetchMovies(
  libraryId: string,
  options: { watched?: boolean; signal?: AbortSignal } = {},
): Promise<MovieSummary[]> {
  const params = new URLSearchParams({ libraryId });
  if (options.watched !== undefined) {
    params.set('watched', String(options.watched));
  }
...
/** Rebuilds the movie catalogue from indexed video files (recovery command). */
export function reindexMovies(libraryId?: string): Promise<{ indexedMovies: number }> {
  return apiFetch<{ indexedMovies: number }>(
    libraryId ? `/movies/reindex?libraryId=${libraryId}` : '/movies/reindex',
    { method: 'POST' },
  );
}
