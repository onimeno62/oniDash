...
export function MoviesPage() {
  ...
  return <div className="space-y-6">
    ...
    {!error && libraryId && continueWatching.length > 0 && !playing && <section aria-label="Continue watching"><h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted">Continue watching</h3><ul className="flex gap-3 overflow-x-auto pb-2">{continueWatching.map((movie) => <li key={movie.id} className="w-64 shrink-0"><MovieCard movie={movie} onPlay={() => setPlaying(movie)} showProgress /></li>)}</ul></section>}
...
function MovieCard({ movie, onPlay, showProgress = false }: { movie: MovieSummary; onPlay: () => void; showProgress?: boolean }) {
