...
export function MediaCollectionsPage({ kind }: { kind: Kind }) {
  ...
  // `AlbumSummary`/`MovieSummary` carry `title`; `ArtistSummary` carries `name`. The chained
  // form used here left the final branch narrowed to `never`, so test the two shapes directly.
  const filtered = useMemo(() => { const q = query.trim().toLowerCase(); return q ? items.filter(x => ('title' in x ? x.title : x.name).toLowerCase().includes(q)) : items; }, [items, query]);
