...
export function MusicHomeDashboardPage() {
  ...
  const continueTrack = player.current?.track ?? historyTracks[0] ?? recentlyAdded[0] ?? null;
  const continueHistory = continueTrack ? history.find(h => h.trackId === continueTrack.id) : undefined;
  const continueProgress = player.current?.track.id === continueTrack?.id ? player.position : (continueHistory?.playedSeconds ?? 0);
  ...
  return <div className="pb-24">
    ...
    {layout === 'standard' ? <div className="space-y-10">
      ...
      {topTracks.length > 0 && <section><SectionHeader title="Most played" count={topTracks.length} onSeeAll={openStudio} /><Carousel>{topTracks.slice(0, 12).map(t => <TrackCard key={t.id} track={t} active={player.current?.track.id === t.id} onPlay={() => play(t)} favorite={favoriteIds.has(t.id)} onFavorite={() => void toggleFavorite(t)} />)}</Carousel></section>}
